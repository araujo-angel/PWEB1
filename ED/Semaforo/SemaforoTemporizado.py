...
class SemaforoTemporizado:
    def __init__(self, estadoInicial:Estado = Estado.Vermelho): #self se refere a propriedade ou ao método
        self.__estadoAtual = estadoInicial
        self.__timer = Temporizador()
        self.__tempoDeTransicao = {Estado.Verde:20, Estado.Amarelo:5, Estado.Vermelho:10}
    ...
    @property
    def tempoDeTransição(self)->dict:
    ...
    def setup(self, tempoVermelho:int, tempoAmarelo:int, tempoVerde:int):
        #a linguagem ja sabe que a multilinha após a definição do metodo será referente a documentação dele
        '''
        Método que permite ajustar o tempo dos estados do semáforo.

        Argumentos
        ----------
        tempoVermelho(int): tempo em seugundos de permanência no vermelho
        tempoAmarelo(int): tempo em seugundos de permanência no amarelo
        tempoVerde(int): tempo em seugundos de permanência no verde
        '''
